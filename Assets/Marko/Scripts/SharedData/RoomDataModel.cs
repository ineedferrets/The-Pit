...
public partial class RoomDataModel : RealtimeModel {
    ...
    public bool gameCompleted {
        get {
            return _cache.LookForValueInCache(_gameCompleted, entry => entry.gameCompletedSet, entry => entry.gameCompleted);
        }
        set {
            if (this.gameCompleted == value) return;
            _cache.UpdateLocalCache(entry => { entry.gameCompletedSet = true; entry.gameCompleted = value; return entry; });
            InvalidateReliableLength();
            FireGameCompletedDidChange(value);
        }
    }
    ...
    private void FireGameCompletedDidChange(bool value) {
        try {
            gameCompletedDidChange?.Invoke(this, value);
        } catch (System.Exception exception) {
            UnityEngine.Debug.LogException(exception);
        }
    }
    ...
    protected override void Read(ReadStream stream, StreamContext context) {
        while (stream.ReadNextPropertyID(out uint propertyID)) {
            switch (propertyID) {
                ...
                case (uint)PropertyID.GameCompleted: {
                    bool previousValue = _gameCompleted;
                    _gameCompleted = (stream.ReadVarint32() != 0);
                    bool gameCompletedExistsInChangeCache = _cache.ValueExistsInCache(entry => entry.gameCompletedSet);
                    if (!gameCompletedExistsInChangeCache && _gameCompleted != previousValue) {
                        FireGameCompletedDidChange(_gameCompleted);
                    }
                    break;
                }
                default: {
